...
impl Analyzer {
    pub fn analyze(entry_path: &Path, entry_text: &str) -> AnalysisResult {
        ...
        for (p, text) in compiler.workspace.sources.clone() {
            sources.insert(p.clone(), text.clone());
        }
        ...
        for (path, text) in &sources {
            ...
            for e in lex_errs {
                let range = lsp::Range {
                    start: super::symbols::offset_to_position(text, e.span().start),
                    end: super::symbols::offset_to_position(text, e.span().end),
                };
                super::symbols::diagnostics_push(
                    &mut diagnostics,
                    path,
                    lsp::Diagnostic {
                        range,
            ...
            let owned_tokens: Vec<(Token<'static>, SimpleSpan)> = tokens_with_spans
                ...
                .map(|(tok, sp)| {
                    (
                        unsafe { std::mem::transmute::<Token<'_>, Token<'static>>(tok) },
                        sp,
                    )
                })
                .collect();
            let tokens_for_parser: Vec<Token<'static>> =
                owned_tokens.iter().map(|(t, _)| t.clone()).collect();
        ...
        AnalysisResult {
            ...
            hir,
            diagnostics,
            token_spans: token_spans_map,
            contexts: typechecker.contexts.clone(),
        }
    }
}
